...
//     const showWelcome = localStorage.getItem("showUserWelcome");
//     if (showWelcome === "true") {
//       alert("WELCOME TO USER DASHBOARD");
//       localStorage.removeItem("showUserWelcome"); // Show only once
//     }
//   });
...
function calculateBMI() {
    ...
    // ✨ Here send to server after BMI calculated and Status determined
    const heightInput = parseFloat(document.getElementById("height").value);
    const weightInput = parseFloat(document.getElementById("weight").value);
    const targetWeightInput = parseFloat(document.getElementById("targetWeight").value);

    sendDataToServer(heightInput, weightInput, targetWeightInput, bmi, status);
}

// 📌 After clicking "Calculate BMI", also add the details to the Saved Profiles Table
document.querySelector('button[onclick="calculateBMI()"]').addEventListener("click", function() {
    setTimeout(() => {  // Wait a tiny moment to make sure BMI and status are updated
...
function addToProfilesTable(height, weight, targetWeight, bmi, status) {
    ...
    newRow.style.color = "black"; // 👉 Force text color to black
...
function sendDataToServer(height, weight, targetWeight, bmi, status) {
    fetch('/save-profile', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ height, weight, targetWeight, bmi, status })
    })
